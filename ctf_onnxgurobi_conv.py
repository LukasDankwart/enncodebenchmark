...
def ctf_evaluate():
    ...
    # ================= Setup ===================
    onnx_path = config.CONV_MNIST_PATH
    ...
    input_node = "input"
    output_node = "output"
    ctf_output_path = "results/final/onnxgurobi/ctf_conv_mnist.csv"

    # ================= Load images and nn predictions
    images, labels = get_mnist_batch(iterations)
    nn_predictions = [int(run_onnx_on_img(onnx_path, np.array(images[i].unsqueeze(0)))) for i in range(images.shape[0])]
    ...
    for i in range(iterations):
        ...
        presolved_model = gurobi_model.presolve()

        presolved_model.write(f"onnxgurobi_conv_debug_{i}.lp")
        # ================= MEASUREMENTS ======================
