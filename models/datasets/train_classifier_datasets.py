...
def train():
    ...
    if TRAIN_DEEP:
        ...
        model = LinearClassifier(X_train.shape[1])
    else:
    ...
    if TRAIN_DEEP:
        ...
        onnx_file_path = f'{data_folder}/classifier_medium.onnx'  # Output path for the ONNX model
        pyt_full_file_path = f'{data_folder}/classifier_full_medium.pt'  # Output path for the ONNX model
        pyt_weights_file_path = f'{data_folder}/classifier_medium.pt'  # Output path for the ONNX model
        report_file_path = f'{data_folder}/confusion_matrix_medium.txt'
    else:
        onnx_file_path = f'{data_folder}/classifier_tiny.onnx'  # Output path for the ONNX model
        pyt_full_file_path = f'{data_folder}/classifier_full_tiny.pt'  # Output path for the ONNX model
        pyt_weights_file_path = f'{data_folder}/classifier_tiny.pt'  # Output path for the ONNX model
    ...
    torch.onnx.export(model, torch.from_numpy(X_train_dummy[0]).float(), onnx_file_path, export_params=True,
                      verbose=True)
    torch.save(model.state_dict(), pyt_weights_file_path)
    torch.save(model, pyt_full_file_path)
